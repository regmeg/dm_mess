...
def multilayer_perceptron(x, params, training):
    ...
    out_layer = tf.matmul(layer_3, params['hout']) + params['bout']
    return out_layer
...
params = {
    ...
    'hout': tf.get_variable("hout", shape=[n_hidden_3, n_classes], dtype=dtype_tf, initializer=tf.contrib.layers.variance_scaling_initializer(factor=2.0,  mode='FAN_IN', uniform=False,  seed=None, dtype=dtype_tf)),
...
with tf.Session() as sess:
    ...
    test_cost, yp_test = sess.run([cost, ypred], feed_dict={xbatch: X_test, ybatch: Y_test, train_pl: True})
    print ("Testing cost=", test_cost)
    print ("r2_score=", r2_score(np.transpose(Y_test), yp_test))
    saver.save(sess, './summaries_movies/' + str(seed) + '/model/',global_step=epoch)